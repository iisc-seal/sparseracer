...
void PrintDbgInfo::printVariableDeclaration(const Value *V) {
  ...
  //WriteAsOperandInternal(Out, V, TypePrinter, 0, 0);
  if (isa<Function>(V)) 
...
bool PrintDbgInfo::runOnFunction(Function &F) {
  ...
  for (DebugInfoFinder::iterator I = Finder.type_begin(),
    ...
    errs() << "Type: ";
    DIType DIT = DIType(*I);
    errs() << "Basic: " << DIT.isBasicType() << "\n";
    errs() << "Derived: " << DIT.isDerivedType() << "\n";
    errs() << "Composite: " << DIT.isCompositeType() << "\n";
    DIT.print(errs());
    errs() << '\n';
  }
  ...
  for (Function::iterator I = F.begin(), E = F.end(); I != E; ++I) {
    ...
    for (BasicBlock::const_iterator i = BB->begin(), e = BB->end();
      ...
      errs()<<"\n--------------------------**********--------------------------\n";
      i->dump();
      errs() << "\n" << "Printing variable decl for instruction " << i->getOpcodeName();
      printVariableDeclaration(i);
      errs() << "\n" << "Printing variable decl for operands \n";
